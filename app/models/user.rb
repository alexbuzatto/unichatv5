...
class User < ApplicationRecord
  ...
  # Returns agents visible to this user within the given account
  # Admins see all agents, regular agents only see teammates
  # @param account [Account] the account to filter agents for
  # @return [ActiveRecord::Relation] filtered users
  def visible_agents(account)
    # SuperAdmins and Administrators see all agents
    account_user = account_users.find_by(account_id: account.id)
    return account.users if account_user&.administrator? || is_a?(SuperAdmin)

    # Regular agents only see teammates from their teams (including admins in those teams)
    my_team_ids = teams.where(account_id: account.id).pluck(:id)

    # If agent has no teams, they see no one except themselves (or potentially nothing depending on policy)
    # But based on requirement "so listar agente do mesmo time", if no team -> no agents.
    return account.users.where(id: id) if my_team_ids.empty?

    # Return agents from same teams
    team_member_ids = TeamMember.where(team_id: my_team_ids).pluck(:user_id).uniq

    account.users.where(id: team_member_ids)
  end
