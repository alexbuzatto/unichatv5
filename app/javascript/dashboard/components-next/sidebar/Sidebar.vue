...
const { isSidebarItemVisible } = usePolicy();
...
// Captain section removed for premium simplification
    {
      name: 'Contacts',
...
// Campaigns and Portals removed for premium simplification
    {
      name: 'Settings',
  ...
  return allItems.filter(item => isSidebarItemVisible(item.name));
});
</script>
...
<template>
  <aside
    ...
    class="bg-n-solid-1 rtl:border-l ltr:border-r border-n-weak flex flex-col text-sm pb-1 fixed top-0 ltr:left-0 rtl:right-0 h-full z-40 transition-transform duration-200 ease-in-out md:static w-[200px] basis-[200px] md:flex-shrink-0 md:ltr:translate-x-0 md:rtl:-translate-x-0"
    :class="[
      {
  ...
  >
    ...
    >
      ...
      />
      <!-- Changelog removed for unichat purification -->
      <div
